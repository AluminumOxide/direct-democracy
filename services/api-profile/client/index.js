...
const client_sign_up = async function({ answer, profile_token }) {
	...
	const { zkpp, salt } = await auth.pake_client_generate_zkpp(profile_id, answer)
...
const client_sign_in = async function({ profile_id, answer }) {
	...
	const client_sesh = await auth.pake_client_derive_proof(salt, profile_id, answer, private_key, key)

	// send proof to server
	const { proof, jwt } = await server.sign_in_finish({ profile_id, key: client_sesh.proof })

	// verify server proof
	await auth.pake_client_verify_proof(public_key, client_sesh, proof)
