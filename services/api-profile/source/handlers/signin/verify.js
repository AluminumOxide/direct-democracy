...
const sign_in_verify = async function(request, reply, db, log) {
	...
	try {
		...
		if(!profile_query || profile_query.length < 1) {
			log.warn(`Profile/Signin/Verify: Failure: ${profile_id} Error: Profile DNE`)
		...
		// handle duplicate profiles - should never happen
			...
			log.error(`Profile/Signin/Verify: Failure: ${profile_id} Error: Double Profile!`)
		...
		// TODO: sign jwt
		const jwt = { profile_id, auth_token, auth_expiry }

		// return server proof and profile jwt
		log.info(`Profile/Signin/Verify: Success: ${profile_id}`)
		return reply.code(200).send({ proof: server_proof, jwt }) 
	
	// handle errors
	} catch(e) {
		log.error(`Profile/Signin/Verify: Failure: ${profile_id} Error: ${e}`)
		return reply.code(500).send(new Error(internal_error))
	}
}
